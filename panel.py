    ...
    <script>
        ...
        function coinRowHtml(c, i) {
            ...
            return `
                <div class="coin-rate-row" data-index="${i}">
                    ...
                    <div style="flex:1 1 100%;min-width:260px"><label>Multiple Rate Tiers</label><textarea class="tier-editor" placeholder="10000,80000,5,5\n81000,100000,6,6">${escapeHtml(formatTierEditor(c.tiers || [], c.price))}</textarea><div class="tier-help">প্রতি লাইনে: MIN,MAX,USER_RATE,ADMIN_RATE — যেমন 10000,80000,5,5</div></div>
        ...
        function collectCoins() {
            return [...document.querySelectorAll('#coinRatesBox .coin-rate-row')].map((row, i) => ({
                ...
                tiers: parseTierEditor(row.querySelector('.tier-editor')?.value, row.querySelector('.rate-price')?.value, row.querySelector('.rate-coins')?.value),
        ...
        function goHome(){
            try { if (window.history.length > 1) { window.history.back(); setTimeout(()=>{ if(location.pathname.startsWith('/admin')) location.href='/'; },400); return; } } catch(e) {}
