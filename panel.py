...
SUPER_ADMIN_ID = 7294314847  # primary admin — cannot be removed, always has access even if DB is down

def get_admin_ids():
    """Live admin ID set: super admin + any admins added from the panel."""
    ids = {SUPER_ADMIN_ID}
    if db:
        try:
            doc = db.collection("system").document("admins").get()
            if doc.exists:
                ids |= {int(x) for x in (doc.to_dict().get("ids") or []) if str(x).isdigit()}
        except Exception as e:
            print("get_admin_ids error:", e)
    return ids
...
DEFAULT_BRANDING = {"helpLineUrl":"https://t.me/Sapportotp","channelUrl":"https://t.me/yourchannel","disclaimer":"","tutorialVideoUrl":"","notice":""}
...
def is_admin_request():
    ...
    return uid.isdigit() and int(uid) in get_admin_ids()
...
@panel.get('/check')
def check_admin():
    """Return whether the supplied Telegram ID is an active admin."""
    uid = request.headers.get("X-Telegram-User-Id", "") or request.args.get("uid", "")
    if not uid.isdigit():
        return jsonify({"ok": True, "isAdmin": False})
    return jsonify({"ok": True, "isAdmin": int(uid) in get_admin_ids()})
...
<section id="branding" class="section"><h2>🎨 Branding</h2><label>Help Line</label><input class="full" id="helpLineUrl"><label>Channel</label><input class="full" id="channelUrl"><label>Tutorial Video URL (optional, if not uploading a file)</label><input class="full" id="tutorialVideoUrl"><label>Or Upload Tutorial Video</label><input type="file" id="tutorialFile" accept="video/*" onchange="uploadTutorial(this)"><div id="tutorialFileName" class="small"></div><div class="spacer"></div><label>📢 Notice (shown as a banner at the top for all users)</label><textarea class="full" id="notice" placeholder="e.g. আজকের জন্য সিস্টেম রক্ষণাবেক্ষণ চলছে..."></textarea><label>Disclaimer</label><textarea class="full" id="disclaimer"></textarea><button class="green" onclick="saveBranding()">Save</button></section>
<section id="admins" class="section"><h2>🛡️ Admins</h2><p class="small">Telegram ID দিয়ে নতুন admin যোগ করুন — তারা এই admin panel এবং /admin bot command ব্যবহার করতে পারবে।</p><div class="row"><input class="full" id="newAdminId" placeholder="Telegram ID (যেমন 123456789)"><div class="spacer"></div><button class="green" onclick="addAdmin()">➕ Add Admin</button></div><div id="adminsBox"></div></section>
...
async function saveBranding(){await post('/admin/update-branding',{helpLineUrl:helpLineUrl.value,channelUrl:channelUrl.value,tutorialVideoUrl:tutorialVideoUrl.value,notice:notice.value,disclaimer:disclaimer.value})}
async function uploadTutorial(input){let f=input.files[0];if(!f)return;tutorialFileName.textContent='Uploading...';let fd=new FormData();fd.append('video',f);try{let r=await fetch('/admin/branding/upload-tutorial',{method:'POST',headers:H(),body:fd});let d=await r.json();if(!d.ok)throw Error(d.error);tutorialVideoUrl.value=d.url;tutorialFileName.textContent='✅ Uploaded: '+f.name+' — click Save to apply';}catch(e){tutorialFileName.textContent='';alert('Video upload failed: '+e.message)}}
...
async function loadAdmins(){try{let r=await fetch('/admin/admins',{headers:H()});let d=await r.json();if(!d.ok)return;renderAdmins(d.admins,d.superAdmin)}catch(e){}}
function renderAdmins(list,superAdmin){adminsBox.innerHTML=list.map(id=>`<div class="row" style="display:flex;align-items:center;justify-content:space-between"><span>${id}${id===superAdmin?' <b style="color:#ffd700">(Primary)</b>':''}</span>${id===superAdmin?'':`<button class="red" onclick="removeAdmin('${id}')">🗑 Remove</button>`}</div>`).join('')||'No admins yet'}
async function addAdmin(){let id=newAdminId.value.trim();if(!id)return;let d=await post('/admin/admins/add',{id});if(d&&d.ok){newAdminId.value='';renderAdmins(d.admins,d.admins[0])}loadAdmins()}
async function removeAdmin(id){if(!confirm('Remove admin '+id+'?'))return;let d=await post('/admin/admins/remove',{id});loadAdmins()}
loadState();loadOrders('pending');loadUsers();loadAdmins();
</script></body></html>'''

@panel.get('/panel')
def admin_panel():
    uid=request.args.get('uid','')
    if not uid.isdigit() or int(uid) not in get_admin_ids(): return denied()
...
def update_branding():
    if not is_admin_request(): return denied()
    if not db:return jsonify({"ok":False,"error":"Database not connected"}),500
    data=request.get_json(silent=True) or {}; b={"helpLineUrl":str(data.get('helpLineUrl',''))[:300],"channelUrl":str(data.get('channelUrl',''))[:300],"tutorialVideoUrl":str(data.get('tutorialVideoUrl',''))[:600],"notice":str(data.get('notice',''))[:500],"disclaimer":str(data.get('disclaimer',''))[:2000]};db.collection('system').document('branding').set(b,merge=True);return jsonify({"ok":True,"branding":b})

@panel.post('/branding/upload-tutorial')
def upload_tutorial_video():
    if not is_admin_request(): return denied()
    import bot
    url=bot.save_upload(request.files.get('video'),bot.VIDEOS_UPLOAD_DIR,bot.ALLOWED_VIDEO_EXT)
    return jsonify({"ok":bool(url),"url":url,"error":None if url else 'Invalid video file (allowed: mp4, mov, webm, mkv, m4v)'}),200 if url else 400
...
def list_admins():
    if not is_admin_request(): return denied()
    return jsonify({"ok": True, "admins": sorted(get_admin_ids()), "superAdmin": SUPER_ADMIN_ID})

@panel.post('/admins/add')
def add_admin():
    if not is_admin_request(): return denied()
    if not db: return jsonify({"ok": False, "error": "Database not connected"}), 500
    new_id = str((request.get_json(silent=True) or {}).get('id', '')).strip()
    if not new_id.isdigit():
        return jsonify({"ok": False, "error": "Enter a valid numeric Telegram ID"}), 400
    ref = db.collection("system").document("admins")
    doc = ref.get()
    ids = set(doc.to_dict().get("ids") or []) if doc.exists else set()
    ids.add(int(new_id))
    ref.set({"ids": list(ids)}, merge=True)
    return jsonify({"ok": True, "admins": sorted(ids | {SUPER_ADMIN_ID})})

@panel.post('/admins/remove')
def remove_admin():
    if not is_admin_request(): return denied()
    if not db: return jsonify({"ok": False, "error": "Database not connected"}), 500
    rem_id = str((request.get_json(silent=True) or {}).get('id', '')).strip()
    if not rem_id.isdigit():
        return jsonify({"ok": False, "error": "Enter a valid numeric Telegram ID"}), 400
    rem_id = int(rem_id)
    if rem_id == SUPER_ADMIN_ID:
        return jsonify({"ok": False, "error": "The primary admin can't be removed"}), 400
    ref = db.collection("system").document("admins")
    doc = ref.get()
    ids = set(doc.to_dict().get("ids") or []) if doc.exists else set()
    ids.discard(rem_id)
