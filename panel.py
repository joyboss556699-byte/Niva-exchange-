...
DEFAULT_COINS = [
    {"id": "top", "name": "TOP COIN", "icon": "💗", "image": "", "price": 4.7, "rate": 1000, "enabled": True},
    {"id": "ns", "name": "NS COIN", "icon": "👤", "image": "", "price": 4.7, "rate": 1000, "enabled": True},
    {"id": "niva", "name": "NIVA COIN", "icon": "🪙", "image": "", "price": 4.7, "rate": 1000, "enabled": True},
]
...
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <div class="toggle-item">
                <div><strong>Refer System</strong><div style="font-size:12px;opacity:.8">Show/hide refer feature</div></div>
    ...
    <script>
        ...
        function addCoinRow() {
            ...
            coins.push({ id: '', name: 'NEW COIN', icon: '🪙', image: '', price: 1, rate: 1000, enabled: true });
        ...
        function renderPayments(methods) {
            ...
            box.innerHTML = methods.map((m, i) => `
                <div class="payment-row" data-index="${i}">
                    <input type="text" class="pay-name" value="${escapeAttr(m.name || '')}" placeholder="Payment method name">
                    <div class="toggle-switch coin-enabled-toggle ${m.enabled ? 'active' : ''}" onclick="this.classList.toggle('active')"></div>
                    <button class="btn-danger btn-small" onclick="removePaymentRow(${i})">🗑</button>
        ...
        function addPaymentRow() {
            const methods = collectPayments();
            methods.push({ id: '', name: 'New Method', enabled: true });
            renderPayments(methods);
        }
        ...
        async function savePayments() {
            const paymentMethods = collectPayments();
            try {
                const response = await fetch('/admin/update-payments', {
                    method: 'POST', headers: authHeaders({ 'Content-Type': 'application/json' }),
                    body: JSON.stringify({ paymentMethods })
                });
                const result = await response.json();
                if (result.ok) { alert('✅ Payment methods আপডেট হয়েছে!'); loadState(); }
                else alert('Error: ' + result.error);
            } catch (error) { alert('Error: ' + error.message); }
        }
...
def get_admin_state():
    ...
    try:
        ...
        features = {"xr7Task": True, "coinSell": True, "history": False, "refer": False, "tutorial": False}
        coins = DEFAULT_COINS
