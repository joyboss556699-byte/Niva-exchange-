...
<body>
    <div class="container">
        ...
        <div class="section manage-section " data-section="transfer">
            ...
            <p class="small-note">এখানে নতুন Transfer ID দিলে User Panel-এ শুধু এটিই দেখাবে। পুরোনো hardcoded ID আর ব্যবহার হবে না। খালি রাখলে User Panel-এ Transfer ID দেখাবে না।</p>
            <button class="btn-success" onclick="saveTransferId()">💾 Save Transfer ID</button>
        </div>
    ...
    <script>
        ...
        async function saveTransferId(){
            const transferId=document.getElementById('transferId').value.trim();
            try{const r=await fetch('/admin/update-transfer-id',{method:'POST',headers:authHeaders({'Content-Type':'application/json'}),body:JSON.stringify({transferId})});const d=await r.json();if(d.ok){alert('✅ Transfer ID saved');loadState();}else alert('Error: '+d.error);}catch(e){alert('Error: '+e.message);}
        }
