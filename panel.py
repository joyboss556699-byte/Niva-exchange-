...
DEFAULT_PAYMENT_METHODS = [
    {"id": "bikas", "name": "bKash", "enabled": True, "image": "", "icon": "💳", "number": ""},
    {"id": "nogod", "name": "Nagad", "enabled": True, "image": "", "icon": "💳", "number": ""},
    {"id": "rokat", "name": "Rocket", "enabled": True, "image": "", "icon": "💳", "number": ""},
    {"id": "binance", "name": "Binance", "enabled": True, "image": "", "icon": "🔶", "number": ""},
    {"id": "other", "name": "Other", "enabled": True, "image": "", "icon": "💳", "number": ""},
]
DEFAULT_BRANDING = {
    "appName": "NIVA EXCHANGE 💦",
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .back-home{background:#263b55;color:#fff;border:1px solid rgba(255,255,255,.25);width:100%;margin-bottom:12px}
        .tier-editor{width:100%;min-height:76px;font-family:monospace;font-size:12px}
        .tier-help{font-size:11px;opacity:.75;margin-top:4px}
        .tier-grid{display:grid;grid-template-columns:repeat(4,minmax(70px,1fr));gap:7px;margin-top:7px}
        .tier-grid input{width:100%}
        .tier-card{padding:9px;border:1px solid rgba(255,255,255,.14);border-radius:9px;background:rgba(0,0,0,.12);margin-top:7px}
        .tier-card strong{display:block;font-size:12px;color:#FFD700;margin-bottom:4px}
...
<body>
    <div class="container">
        ...
        <div class="section manage-section " data-section="history">
            <h2>🕘 User Sell History</h2>
            <button class="btn-primary btn-small" onclick="loadHistory()">🔄 Refresh History</button>
            <div id="historyBox"><p class="small-note">Loading...</p></div>
        </div>
        ...
        <div class="section manage-section " data-section="transfer">
            ...
            <div id="transferProfilePreview" style="margin:8px 0"></div>
             <button type="button" class="btn-danger btn-small" id="removeTransferProfileBtn" onclick="removeTransferProfileImage()">🗑 Remove Profile Picture</button>
        ...
        <div class="section manage-section " data-section="branding">
            ...
            <label class="field-label">📛 App Name (splash screen ও উপরে হেডারে দেখাবে)</label>
            <input type="text" id="appName" placeholder="NIVA EXCHANGE 💦">
            ...
            <label class="field-label">🎬 Splash / Intro Image (bot start করলে ৩ সেকেন্ডের জন্য দেখাবে)</label>
            <div id="splashImagePreviewBox" style="margin-bottom:8px"></div>
            <input type="file" id="splashImageFile" accept="image/*">
            <input type="hidden" id="splashImage">
    ...
    <script>
        ...
        async function loadState() {
            try {
                ...
                document.getElementById('appName').value = data.branding?.appName || '';
                ...
                document.getElementById('splashImage').value = data.branding?.splashImage || '';
                renderSplashImagePreview(data.branding?.splashImage || '');
            } catch (error) {
        ...
        function coinRowHtml(c, i) {
            ...
            return `
                <div class="coin-rate-row" data-index="${i}">
                    ...
                    <div><label>Default মূল্য (টাকা)</label><input type="number" step="0.01" class="rate-price" value="${c.price}"></div>
                    <div><label>কয়েন (per rate)</label><input type="number" step="1" class="rate-coins" value="${c.rate}"></div>
                    <div style="flex:1 1 100%;min-width:260px"><label>Rate 1 / Rate 2 / Rate 3</label>${tierCardsHtml(c.tiers || [], c.price)}<div class="tier-help">Amount range এবং প্রতি 1,000 coin-এর দাম দিন। যেমন Rate 1: 1000–80000 = 5, Rate 2: 80001–100000 = 6।</div></div>
                    ...
                    ${c.image ? '<button type="button" class="btn-danger btn-small" onclick="removeCoinImage('+i+')">🗑 Remove Image</button>' : ''}
                </div>`;
        }

        function tierCardsHtml(tiers, fallbackPrice) {
            const rows = Array.isArray(tiers) ? tiers.slice(0, 3) : [];
            while (rows.length < 3) rows.push({min:'',max:'',userPrice:'',adminPrice:''});
            return rows.map((t, n) => {
                const price = t.userPrice ?? t.price ?? fallbackPrice ?? '';
                return `<div class="tier-card">
                    <strong>RATE ${n + 1}</strong>
                    <div class="tier-grid">
                        <div><label>Min Coin</label><input type="number" class="tier-min" value="${t.min ?? ''}" placeholder="1000"></div>
                        <div><label>Max Coin</label><input type="number" class="tier-max" value="${t.max ?? ''}" placeholder="80000"></div>
                        <div><label>Price / 1000</label><input type="number" step="0.01" class="tier-price" value="${price}" placeholder="5"></div>
                        <div><label>Admin Price</label><input type="number" step="0.01" class="tier-admin-price" value="${t.adminPrice ?? price}" placeholder="5"></div>
                    </div>
                </div>`;
            }).join('');
        }
        ...
        function formatTierEditor(tiers, fallbackPrice) {
            if (!Array.isArray(tiers) || !tiers.length) return '';
            return tiers.map(t => `${Number(t.min||1)},${t.max==null?'':Number(t.max)},${Number(t.userPrice ?? t.price ?? fallbackPrice ?? 0)},${Number(t.adminPrice ?? t.userPrice ?? t.price ?? fallbackPrice ?? 0)}`).join('\\n');
        }
        function parseTierEditor(value, fallbackPrice, fallbackRate) {
            const lines = String(value || '').split(/\\r?\\n/).map(x=>x.trim()).filter(Boolean);
            const tiers=[];
            for (const line of lines) {
                const parts=line.split(',').map(x=>x.trim());
                if(parts.length<3) continue;
                const min=Number(parts[0]), max=(parts[1]===''||parts[1].toLowerCase()==='null'||parts[1]==='∞')?null:Number(parts[1]);
                const userPrice=Number(parts[2]); const adminPrice=parts.length>=4 && parts[3]!=='' ? Number(parts[3]) : userPrice;
                if(Number.isFinite(min)&&min>0&& (max===null || (Number.isFinite(max)&&max>=min)) && Number.isFinite(userPrice)&&userPrice>=0 && Number.isFinite(adminPrice)&&adminPrice>=0) tiers.push({min,max,price:userPrice,userPrice,adminPrice});
            }
            return tiers.length ? tiers : [{min:1,max:null,price:Number(fallbackPrice)||0,userPrice:Number(fallbackPrice)||0,adminPrice:Number(fallbackPrice)||0}];
        }

        function collectTierCards(row, fallbackPrice) {
            const cards = [...row.querySelectorAll('.tier-card')];
            const tiers = cards.map(card => {
                const minRaw = card.querySelector('.tier-min')?.value.trim() || '';
                const maxRaw = card.querySelector('.tier-max')?.value.trim() || '';
                const priceRaw = card.querySelector('.tier-price')?.value.trim() || '';
                const adminRaw = card.querySelector('.tier-admin-price')?.value.trim() || '';
                if (!minRaw && !maxRaw && !priceRaw && !adminRaw) return null;
                const min = Number(minRaw), max = maxRaw === '' ? null : Number(maxRaw);
                const userPrice = Number(priceRaw), adminPrice = adminRaw === '' ? userPrice : Number(adminRaw);
                if (!Number.isFinite(min) || min <= 0 || (max !== null && (!Number.isFinite(max) || max < min)) || !Number.isFinite(userPrice) || userPrice < 0 || !Number.isFinite(adminPrice) || adminPrice < 0) return null;
                return {min, max, price:userPrice, userPrice, adminPrice};
            }).filter(Boolean);
            return tiers.length ? tiers : [{min:1,max:null,price:Number(fallbackPrice)||0,userPrice:Number(fallbackPrice)||0,adminPrice:Number(fallbackPrice)||0}];
        }
        ...
        function collectCoins() {
            return [...document.querySelectorAll('#coinRatesBox .coin-rate-row')].map((row, i) => ({
                ...
                tiers: collectTierCards(row, row.querySelector('.rate-price')?.value),
        ...
        function renderPayments(methods) {
            ...
            box.innerHTML = methods.map((m, i) => `
                <div class="payment-row" data-index="${i}">
                    ...
                    <div style="flex:1"><input type="text" class="pay-name" value="${escapeAttr(m.name || '')}" placeholder="Payment method name">
                    <input type="text" class="pay-number" value="${escapeAttr(m.number || '')}" placeholder="Admin payment number / account">
        ...
        function collectPayments() {
            return [...document.querySelectorAll('#paymentsBox .payment-row')].map((row, i) => ({
                ...
                number: row.querySelector('.pay-number')?.value.trim() || '',
                enabled: row.querySelector('.coin-enabled-toggle').classList.contains('active')
            }));
        }
        function addPaymentRow() { const methods=collectPayments(); methods.push({id:'',name:'New Method',enabled:true,image:'',icon:'💳',number:''}); renderPayments(methods); }
        ...
        function renderSplashImagePreview(url){
            const box=document.getElementById('splashImagePreviewBox');
            if(!box)return;
            box.innerHTML=url ? `<img src="${escapeAttr(url)}" style="width:120px;height:120px;border-radius:14px;object-fit:cover;border:2px solid #31b8f4">` : '<span class="small-note">কোনো splash image সেট নেই</span>';
        }
        document.addEventListener('change', async function(e){
            ...
            if(e.target && e.target.id==='splashImageFile'){
                const file=e.target.files[0]; if(!file)return;
                const fd=new FormData(); fd.append('image',file);
                try{const r=await fetch('/admin/splash/upload',{method:'POST',headers:authHeaders(),body:fd});const d=await r.json();if(!d.ok)throw new Error(d.error);document.getElementById('splashImage').value=d.url;renderSplashImagePreview(d.url);alert('✅ Splash image uploaded — এখন নিচের Save Branding চাপুন');}
                catch(err){alert('Upload error: '+err.message);}
            }
        });
        function removeCoinImage(index) {
            const coins = collectCoins();
        ...
        function removeTransferProfileImage() {
            document.getElementById('transferProfileImage').value = '';
            renderTransferProfilePreview('');
            alert('🗑 Profile picture removed from settings. এখন Save Transfer Account চাপুন।');
        }
        ...
        async function copyText(value){
            ...
            if(!value){alert('কপি করার মতো তথ্য নেই');return;}
            try{await navigator.clipboard.writeText(value);alert('✅ Copied');}
            catch(e){prompt('Copy করুন:',value);}
        }
        function goHome(){
            // Return to the exact WebApp page that opened the admin panel.
        ...
        async function saveBranding() {
            const payload = {
                ...
                splashImage: document.getElementById('splashImage').value
            };
            try {
        ...
        async function loadHistory() {
            ...
            try {
                const response = await fetch('/admin/history', { headers: authHeaders() });
                const data = await response.json();
                if (!data.ok) { box.innerHTML = '<p class="small-note">Error: ' + data.error + '</p>'; return; }
                if (!data.history.length) { box.innerHTML = '<p class="small-note">কোনো history নেই।</p>'; return; }
                box.innerHTML = data.history.map(h => {
                    const cls = h.status === 'approved' ? 'status-approved' : h.status === 'rejected' ? 'status-rejected' : 'status-pending';
                    return `<div class="order-card"><div class="order-meta"><strong>${h.coin || 'COIN'}</strong> — ${(h.quantity || 0).toLocaleString()} coins — <strong>${Number(h.total_amount || 0).toFixed(2)} টাকা</strong> <span class="order-status ${cls}">${h.status || 'pending'}</span><br>User: @${h.username || '—'} (ID: ${h.user_id || '—'})<br>Payment: ${h.payment_method || '—'}<br>Time: ${h.timestamp || '—'}</div></div>`;
                }).join('');
            ...
            return `
                <div class="order-card">
                    <div class="order-meta">
                        ...
                        Payment: ${o.payment_method || '—'} → ${o.account_number || o.user_payment_number || '—'}<br>
                        <button class="btn-primary btn-small" type="button" onclick="copyText(${JSON.stringify(String(o.account_number || o.user_payment_number || ''))})">📱 Copy Number</button>
                        Coupon: ${o.coupon_code || '—'}<br>
                        <button class="btn-primary btn-small" type="button" onclick="copyText(${JSON.stringify(String(o.coupon_code || ''))})">🎟 Copy Coupon</button><br>
                        Time: ${o.timestamp || '—'}
                    </div>
                    ${img}
                    ${actions}
...
def get_admin_state():
    ...
    try:
        ...
        if db:
            ...
            if coins_doc.exists and "list" in (coins_doc.to_dict() or {}):
                coins = coins_doc.to_dict().get("list") or []

            payments_doc = db.collection("system").document("payments").get()
            if payments_doc.exists and "list" in (payments_doc.to_dict() or {}):
                payment_methods = payments_doc.to_dict().get("list") or []
...
def update_coins():
    ...
    try:
        ...
        for c in coins:
            ...
            incoming_tiers = c.get("tiers") if isinstance(c.get("tiers"), list) else []
            if not incoming_tiers:
                incoming_tiers = [{"min": 1, "max": None, "price": price, "userPrice": price, "adminPrice": price}]
            c = dict(c)
            c["tiers"] = incoming_tiers
            clean_coins.append({
                ...
                "tiers": [
                    {
                        "min": float(t.get("min", 1) or 1),
                        "max": (None if t.get("max") in (None, "", "null", "∞") else float(t.get("max"))),
                        "price": float(t.get("price", t.get("userPrice", price)) or 0),
                        "userPrice": float(t.get("userPrice", t.get("price", price)) or 0),
                        "adminPrice": float(t.get("adminPrice", t.get("userPrice", t.get("price", price))) or 0),
                    }
        ...
        if not db:
            return jsonify({"ok": False, "error": "Database not connected"}), 500
        db.collection("system").document("coins").set({"list": clean_coins, "updatedAt": datetime.now(timezone.utc).isoformat()})
        response = jsonify({"ok": True, "coins": clean_coins})
    ...
    try:
        ...
        for m in methods:
            clean_methods.append({
                ...
                "number": str(m.get("number", m.get("mobile", m.get("phone", ""))) or "")[:100],
            })

        if not db:
            return jsonify({"ok": False, "error": "Database not connected"}), 500
        db.collection("system").document("payments").set({"list": clean_methods, "updatedAt": datetime.now(timezone.utc).isoformat()})
        response = jsonify({"ok": True, "paymentMethods": clean_methods})
        response.headers["Cache-Control"] = "no-store"
        return response
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def update_binance_rate():
    ...
    try:
        ...
        db.collection("system").document("config").set({"binanceRate": rate, "updatedAt": datetime.now(timezone.utc).isoformat()}, merge=True)
        response = jsonify({"ok": True, "binanceRate": rate})
        response.headers["Cache-Control"] = "no-store"
        return response
...
def update_branding():
    ...
    try:
        data = request.get_json() or {}
        branding = {
            "appName": str(data.get("appName", "") or "NIVA EXCHANGE 💦")[:80],
            ...
            "splashImage": str(data.get("splashImage", "") or "")[:500],
        }
        if not db:
            return jsonify({"ok": False, "error": "Database not connected"}), 500
...
def admin_history():
    ...
    try:
        ...
        return jsonify({"ok": True, "history": rows[:100]})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
