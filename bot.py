...
ADMIN_IDS = {int(x) for x in os.getenv("ADMIN_ID", "7294314847").split(",") if x.strip().isdigit()}
...
def init_firebase():
    ...
    if not FIREBASE_CREDENTIALS.get("private_key") or not FIREBASE_CREDENTIALS.get("client_email"):
        print("❌ Firebase env vars not set (FIREBASE_PRIVATE_KEY / FIREBASE_CLIENT_EMAIL etc). "
