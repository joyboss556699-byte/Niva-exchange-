...
_SECRETS_FILE = os.path.join(BASE_DIR, "secrets.json")
_secrets_cache = None
    ...
    if os.path.exists(_SECRETS_FILE):
        try:
            with open(_SECRETS_FILE, "r", encoding="utf-8") as f:
                _secrets_cache = json.load(f) or {}
        except Exception as e:
            print("⚠️ Could not read secrets.json:", e)
    return _secrets_cache

def secret(key, default=""):
    """Read a secret: real env var first, then secrets.json (for hosts with no env-var UI)."""
    val = os.getenv(key)
    if val:
        return val
    return str(_load_secrets_file().get(key, default))

# ============================================================
# NIVA EXCHANGE - CONFIGURATION
# ============================================================
BOT_TOKEN = secret("BOT_TOKEN", "")
WEBAPP_URL = secret("WEBAPP_URL", "https://fi6.bot-hosting.net:22464")
SERVER_PORT = int(secret("PORT", secret("SERVER_PORT", "22464")))
ADMIN_IDS = {int(x) for x in secret("ADMIN_ID", "7294314847").split(",") if x.strip().isdigit()}

# ============================================================
# FIREBASE
# ============================================================
# NOTE: all values below MUST come from environment variables now.
# The old hardcoded service-account key was exposed in this file and
# must be treated as compromised — rotate it in the Firebase console
# (Project Settings -> Service Accounts -> Generate new private key),
# then set these as env vars on your host. Do NOT put real values back
# into this file, especially if it is ever pushed to a public repo.
FIREBASE_CREDENTIALS = {
    "type": "service_account",
    "project_id": secret("FIREBASE_PROJECT_ID", ""),
    "private_key_id": secret("FIREBASE_PRIVATE_KEY_ID", ""),
    "private_key": secret("FIREBASE_PRIVATE_KEY", "").replace("\\n", "\n"),
    "client_email": secret("FIREBASE_CLIENT_EMAIL", ""),
    "client_id": secret("FIREBASE_CLIENT_ID", ""),
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": secret("FIREBASE_CLIENT_CERT_URL", ""),
...
def init_firebase():
    ...
    if not FIREBASE_CREDENTIALS.get("private_key") or not FIREBASE_CREDENTIALS.get("client_email"):
        print("❌ Firebase credentials not set. Set them as environment variables, OR create a "
              f"secrets.json file next to bot.py at: {_SECRETS_FILE}\n"
              "Coins, images, orders, and Instagram/transfer settings will NOT save until these are set.")
        return None
    try:
...
VIDEOS_UPLOAD_DIR = os.path.join(UPLOADS_DIR, "videos")
...
def save_upload(file_storage, folder, allowed_ext=None):
    """Save an uploaded file safely, return its public URL path or None."""
    ...
    if folder == ORDERS_UPLOAD_DIR:
        subfolder = "orders"
    elif folder == PAYMENTS_UPLOAD_DIR:
        subfolder = "payments"
    elif folder == VIDEOS_UPLOAD_DIR:
        subfolder = "videos"
    else:
        subfolder = "coins"
...
from panel import panel, set_database, DEFAULT_COINS, DEFAULT_PAYMENT_METHODS, DEFAULT_BRANDING, get_admin_ids as _panel_admin_ids

def current_admin_ids():
    """Static env-based admins + any admins added live from the panel."""
    return ADMIN_IDS | _panel_admin_ids()
set_database(db)
...
def serve_upload(subfolder, filename):
    folders = {
        "orders": ORDERS_UPLOAD_DIR,
        "payments": PAYMENTS_UPLOAD_DIR,
        "videos": VIDEOS_UPLOAD_DIR,
        "coins": COINS_UPLOAD_DIR,
    }
    folder = folders.get(subfolder, COINS_UPLOAD_DIR)
...
def get_config():
    ...
    try:
        ...
        transfer = {"transferId":"","transferUsername":"","transferProfileImage":""}
        if db:
            transfer_doc = db.collection("system").document("transfer").get()
            if transfer_doc.exists:
                transfer.update(transfer_doc.to_dict() or {})
        ...
        # Normalize legacy payment ids/names while preserving saved icons.
        aliases = {"bikas":"bkash","nogod":"nagad","rokat":"rocket"}
        normalized_payments = []
        for m in payment_methods:
            pm = dict(m)
            old_id = str(pm.get("id", ""))
            pm["id"] = aliases.get(old_id, old_id)
            pm.setdefault("icon", "💳")
            pm.setdefault("image", "")
            normalized_payments.append(pm)
        return jsonify({"ok": True, "coins": normalized_coins, "paymentMethods": normalized_payments, "branding": branding, "binanceRate": binance_rate, "transferId": str(transfer.get("transferId", "")), "transferUsername": str(transfer.get("transferUsername", "")), "transferProfileImage": str(transfer.get("transferProfileImage", "")), "sendFee": 0})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def build_admin_panel_url(admin_id: int) -> str:
    """Build the admin panel URL with the current Telegram user ID."""
    ...
    try:
        ...
        query["uid"] = str(admin_id)
        return urlunsplit((parts.scheme, parts.netloc, parts.path, urlencode(query), parts.fragment))
    except Exception:
        return base.rstrip("/") + "/admin/panel?uid=" + str(admin_id)


async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Open the protected admin panel for an approved Telegram admin."""
    ...
    if admin_id not in current_admin_ids():
        await update.message.reply_text(
            "⛔ আপনি Admin নন।\n\nএই command শুধুমাত্র অনুমোদিত Admin-দের জন্য।"
        )
        return

    panel_url = build_admin_panel_url(admin_id)
    if not panel_url:
        await update.message.reply_text("⚠️ Admin Panel URL সেট করা নেই।")
        return
    ...
    await update.message.reply_text(
        "🔐 NIVA EXCHANGE — ADMIN PANEL\n\n"
        f"👤 Admin ID: {admin_id}\n"
        "✅ Admin access verified.\n\n"
        "নিচের button-এ চাপ দিয়ে Admin Panel খুলুন।",
...
async def on_order_decision(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.effective_user or update.effective_user.id not in current_admin_ids():
    ...
    try:
        ...
        if user_id:
            if decision == "approved":
                if order_data.get("payment_method") == "binance":
                    text = (f"✅ আপনার অর্ডার (কয়েন: {coin}, পরিমাণ: {quantity}, "
                            f"পেমেন্ট: {order_data.get('usdt_amount', 0)} USDT) অ্যাডমিন গ্রহণ করেছেন।")
                else:
                    text = (f"✅ আপনার অর্ডার (কয়েন: {coin}, পরিমাণ: {quantity}, "
...
def notify_admin_new_order(order_id, order_data):
    amount_line = (f"Total: {order_data.get('usdt_amount')} USDT (Rate: {order_data.get('binance_rate')})\n"
                   if order_data.get('payment_method') == 'binance'
                   else f"Total: {order_data.get('total_amount')} টাকা\n")
    caption = (
        f"🆕 নতুন সেল অর্ডার\n\n"
        f"Order ID: {order_id}\n"
        f"User: @{order_data.get('username') or '—'} (ID: {order_data.get('user_id')})\n"
        f"Instagram: {order_data.get('instagram_username') or '—'}\n"
        f"Coin: {order_data.get('coin')}\n"
        f"Quantity: {order_data.get('quantity')}\n"
        + amount_line
        + f"Payment: {order_data.get('payment_method')} → {order_data.get('account_number') or '—'}\n"
...
async def run_bot():
    ...
    if not BOT_TOKEN or BOT_TOKEN == "PUT_BOT_TOKEN_HERE":
        print(f"⚠️ Set BOT_TOKEN as an env var, or add it to secrets.json at: {_SECRETS_FILE}")
        return
