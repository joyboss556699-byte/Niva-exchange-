...
# ============================================================
BOT_TOKEN = os.getenv("BOT_TOKEN", "8796779450:AAHIQcqjV3WiSkFSGSKF7-btmTDh9S2B8fM")
...
ADMIN_IDS = {int(os.getenv("ADMIN_ID", "7494199995"))}
...
FIREBASE_CREDENTIALS = {
    ...
    "project_id": os.getenv("FIREBASE_PROJECT_ID", "nivaexchangeowner"),
    "private_key_id": os.getenv("FIREBASE_PRIVATE_KEY_ID", "f608c0c854615ff6bce8e2d9a4584933cee00b27"),
    "private_key": os.getenv(
        "FIREBASE_PRIVATE_KEY",
        "-----BEGIN PRIVATE KEY-----\n"
        "MIIEvwIBADANBgkqhkiG9w0BAQEFAASCBKkwggSlAgEAAoIBAQDhMqsHhSw86oLH\n"
        "C9Y/6H8I7ysyQk3pjX7NMcZBvM/s1w5GMfpHIOSKGCjKvCNxilmzeaGGYRis8goK\n"
        "xS/2ns4185+5BCAsuhmJWB1tpnBMYSdq4NwWyqRwhS1tQjQCyKMXZeNybKuZkk2O\n"
        "xd8mIqlDP9L9oegA+3sMyhCO7+fb5NFYVkKBSvzFWytIMtSyBVUqlcs3n1Ieimcc\n"
        "rU9olS97Xy8RvOpKE2744p4CwH8eQ/rqp0d43odxvdbMSO0PMQhwwE3ZNQu5P3o2\n"
        "zOjcry+OtKMrwlwaePQuyn51cKWRqdaKiofAFh0gtJwsaqWCPLRIf1Q2egu154oU\n"
        "rKWvBj4FAgMBAAECggEAb/2iFzXPl6WqDrGgxRxqnHdQPIjDtUuEDShdJ1Dt+CLE\n"
        "uvzcsaW2ccRs+NFSMQMz87CftrOvsUYW319LqHI2K5dxzXEzTPrpeydiTybPvvQr\n"
        "xZd4bC8vUrOwMR8UvXOow5iMYVmQhuk+f6uAezcOhN6HH1H2jhKCEYylamf9Hpk/\n"
        "0bGugKFcm/cT76AlT9EyzP7feId2P3sHVU/hBDuEahTrx8tEcEpXHJUZQZGoWvME\n"
        "K7muC9pRLWhO0lWwq4Ygz0s8XuNvKONHJutKqzPbH+Z19nG/jPCOI4zO/PljhY74\n"
        "TlDgr0E7c3essqh+NFLPePEXQwGR1k2JzXHmXnRqqwKBgQD5s9PGx7iXGbKvD+Lz\n"
        "ZF8U9+04P6bWJ1gZbdWgftmhIoJdthwAL0gegqh62VcBbc3ZpbCHJA/i7fHL2mBf\n"
        "7lCg/cPubliIrBMHYU5xozYMdzmzcJr+31pkhsNjqHOwyNwrO7p3iTdFRNyt7pLo\n"
        "2TLzATxvepoT9jyb0CvKz7XhtwKBgQDm4KFh8GRCkpuRm2VYBm4Gbp3nUnts96FZ\n"
        "LQI67JJnTtVRSptY9orEp5nMZG4LHkPLn2G6TGxa7N/29Q/gWDIWamdyvEAhUcQn\n"
        "8dWoRx/pZ2v/Z/U5GeyTkziE3KvT7JkLAzVyAnS8Ija2m3ZyyIkoXOabEhs1wGs3\n"
        "Mxp1DDKuIwKBgQDBQX5sYHLq7wauGtuJiGtWbTP5zrmvETBMLgeiecqxvWWsZZ9y\n"
        "PSXSB0miGsgN4Q/qnSPs0+o/tezn2xGv7WAV2ZFk3Z5D2Pm5QoRVyv2v1C6iREqR\n"
        "bAgnuW1Lmjo/3pUYGNgOX/KY99zb+MNW15NpI9HEbd0GChOKfCkIhI2+LQKBgQCS\n"
        "bXNNJ2afJlaOUa1D18geQjfZh2GKab+mTDp23qW36bK3PQcrtCmssdOEY0m7uXUs\n"
        "Bqi3vFKYlFUzTpfEJIuX1eJITYaq2y36W5GziXB+2r4PHYLUtVFrA1s8QKyl2w/x\n"
        "gRZ7eX1J+LUUuaK1B+SdedY0EZa6T5BwUPP9uj/GWwKBgQCffuD3pwUkKfkPoTP7\n"
        "9VPmOTBgX6PRAD438VdzcZW+vAWMRJSukyRDeGcbGUD5qKJ5vTrQlYfvT5iKyyZF\n"
        "7U9NFtWOpxwA9NpbrtMWZ2jmVtDW85SKetE+kTjX5dpYcFipnCCmPpUErY4ZLrXr\n"
        "jpcNEDceKuTbqnXUcg9d1ACJOg==\n"
        "-----END PRIVATE KEY-----\n"
    ),
    "client_email": os.getenv(
        "FIREBASE_CLIENT_EMAIL",
        "firebase-adminsdk-fbsvc@nivaexchangeowner.iam.gserviceaccount.com",
    ),
    "client_id": os.getenv("FIREBASE_CLIENT_ID", "111595205883731748567"),
    ...
    "client_x509_cert_url": os.getenv(
        ...
        "firebase-adminsdk-fbsvc%40nivaexchangeowner.iam.gserviceaccount.com",
    ),
...
def add_cors_headers(response):
    ...
    response.headers["Access-Control-Allow-Headers"] = "Content-Type, X-Telegram-User-Id, Cache-Control, Pragma"
    ...
    try:
        ...
        if db:
            ...
            if features_doc.exists:
                features.update(features_doc.to_dict() or {})
        response = jsonify({"ok": True, "coins": normalized_coins, "paymentMethods": payment_methods, "branding": branding, "features": features, "binanceRate": binance_rate, "transferId": transfer_id, "transferUsername": transfer_username, "transferProfileImage": transfer_profile_image, "sendFee": 0, "configVersion": datetime.now(timezone.utc).isoformat()})
...
def get_user_history():
    """Return the logged-in user's persistent sell/order history."""
    try:
...
def get_leaderboard():
    """Persistent leaderboard based on approved coin sales."""
    try:
        if not db:
            return jsonify({"ok": True, "leaders": []})
        totals = {}
        for doc in db.collection("orders").stream():
            o = doc.to_dict() or {}
            if str(o.get("status", "pending")) != "approved":
                continue
            uid = str(o.get("user_id", "")).strip()
            if not uid:
                continue
            item = totals.setdefault(uid, {"user_id": uid, "username": o.get("username") or "", "coins": 0, "orders": 0})
            item["coins"] += int(o.get("quantity", 0) or 0)
            item["orders"] += 1
            if not item["username"] and o.get("username"):
                item["username"] = o.get("username")
        leaders = sorted(totals.values(), key=lambda x: (x["coins"], x["orders"]), reverse=True)[:100]
...
def save_user():
    ...
    user_data["pending_count"] = int(pending_count)
    user_data["approved_count"] = int(approved_count)
...
def create_order():
    try:
        ...
        is_binance = payment_method.strip().lower() == "binance" or "binance" in payment_method.strip().lower()
        submitted_usdt = str(data.get("usdt_amount", "") or "").strip()
        try:
            usdt_amount = float(submitted_usdt) if is_binance and submitted_usdt else (total_amount / binance_rate if is_binance and binance_rate > 0 else 0)
        except Exception:
            usdt_amount = total_amount / binance_rate if is_binance and binance_rate > 0 else 0
        ...
        order_data = {
            ...
            "user_payment_number": data.get("user_payment_number", data.get("account_number", "")),
            "payment_method_number": data.get("payment_method_number", ""),
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.effective_user or not update.message:
        return
    # Telegram's in-app browser aggressively caches the WebApp page. Appending a
    # fresh version each time /start runs forces it to fetch the latest deployed
    # index.html instead of showing an old cached version.
    fresh_webapp_url = WEBAPP_URL + ("&" if "?" in WEBAPP_URL else "?") + "v=" + str(int(time.time()))
    keyboard = [[InlineKeyboardButton("💦 OPEN NIVA EXCHANGE", web_app=WebAppInfo(url=fresh_webapp_url))]]
...
def process_order_decision(order_id, decision):
    ...
    try:
        ...
        if user_id:
            receive_number = order_data.get("user_payment_number") or order_data.get("account_number") or "—"
            coupon = order_data.get("coupon_code") or "—"
            method = order_data.get("payment_method") or "—"
            is_binance = str(method).strip().lower() == "binance" or "binance" in str(method).strip().lower()
            amount_text = f"{float(order_data.get('usdt_amount', 0) or 0):.2f} USDT" if is_binance else f"{total} টাকা"
            if decision == "approved":
                text = (f"✅ আপনার অর্ডার Approved হয়েছে।\n\n"
                        f"Coin: {coin}\nQuantity: {quantity:,}\nTotal: {amount_text}\n"
                        f"Payment: {method}\nReceive Number: {receive_number}\nCoupon: {coupon}\n\n"
                        f"Admin আপনার payment পাঠানোর জন্য order approve করেছেন।")
            else:
                text = (f"❌ আপনার অর্ডার Rejected হয়েছে।\n\n"
                        f"Coin: {coin}\nQuantity: {quantity:,}\nPayment: {method}\n"
                        f"Receive Number: {receive_number}\nCoupon: {coupon}\n\n"
                        f"বিস্তারিত জানতে Help Line-এ যোগাযোগ করুন।")
...
def notify_admin_new_order(order_id, order_data):
    # Photo captions are inconvenient to copy on some Telegram Android clients.
    # Send the sensitive order fields again as a normal code-formatted message.
    payment_method = str(order_data.get('payment_method') or '').strip().lower()
    is_binance = payment_method == 'binance' or 'binance' in payment_method
    total_value = order_data.get('usdt_amount') if is_binance else order_data.get('total_amount')
    total_label = 'USDT' if is_binance else 'টাকা'
    rate_line = f"Binance Rate: 1 USDT = {order_data.get('binance_rate')} BDT\n" if is_binance else ""
    caption = (
        f"🆕 নতুন সেল অর্ডার\n\n"
        f"Order ID: {order_id}\n"
        ...
        f"Total: {total_value} {total_label}\n"
        f"{rate_line}"
        f"Payment: {order_data.get('payment_method')}\n"
        f"User Receive Number: `{order_data.get('user_payment_number') or order_data.get('account_number') or '—'}`\n"
    ...
    for admin_id in ADMIN_IDS:
        try:
            ...
            receive_number = str(order_data.get('user_payment_number') or order_data.get('account_number') or '—')
            coupon_code = str(order_data.get('coupon_code') or '—')
            copy_message = (
                "📋 <b>COPY DETAILS</b>\n\n"
                "📱 Receive Number: <code>" + html.escape(receive_number) + "</code>\n"
                "🎟 Coupon Code: <code>" + html.escape(coupon_code) + "</code>"
            )

            # Telegram's native CopyTextButton copies the exact value with one tap.
            if CopyTextButton:
                copy_keyboard = InlineKeyboardMarkup([
                    [InlineKeyboardButton("📋 Copy Number", copy_text=CopyTextButton(receive_number))],
                    [InlineKeyboardButton("🎟 Copy Coupon", copy_text=CopyTextButton(coupon_code))],
                ])
                _run_coro(_send_message(admin_id, copy_message, copy_keyboard, parse_mode='HTML'))
            else:
                # Fallback for older python-telegram-bot versions.
                _run_coro(_send_message(admin_id, copy_message + "\n\n⚠️ One-tap copy requires a newer python-telegram-bot version.", parse_mode='HTML'))
        except Exception as e:
...
def send_broadcast(text):
    ...
    for doc in db.collection("users").stream():
        ...
        if not uid:
            continue
        try:
            if _run_coro(_send_message(uid, text)):
                sent += 1
            else:
                failed += 1
        except Exception:
