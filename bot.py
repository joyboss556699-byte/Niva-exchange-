...
def save_upload(file_storage, folder):
    ...
    subfolder = "orders" if folder == ORDERS_UPLOAD_DIR else "coins"
...
def serve_upload(subfolder, filename):
    folder = ORDERS_UPLOAD_DIR if subfolder == "orders" else COINS_UPLOAD_DIR
...
def get_config():
    ...
    try:
        ...
        return jsonify({"ok": True, "coins": normalized_coins, "paymentMethods": payment_methods, "branding": branding})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def save_user():
    ...
    return jsonify({"ok": True, "user": user_data, "blocked": blocked})
